...
class pointnet2(nn.Module):
    ...
    ):  # normal_channel = True so that the input is 6 channels
    ...
    def forward(self, xyz):
        ...
        if self.normal_channel:
            norm = xyz[:, 3:, :]  # normal is afterwads channels
            xyz = xyz[:, :3, :]  # xyz is front 3 channels
        else:
            norm = None
...
def resnet50(pretrained=False, progress=True, **kwargs):
    ...
    if pretrained:
        ...
        pre_train_model = {
            ...
            if k in model_dict and not ("branch_" in k)
        }
        model_dict.update(pre_train_model)
        model.load_state_dict(model_dict)
...
def resnext50_32x4d(pretrained=False, progress=True, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 6, 3], pretrained, progress, **kwargs)
    if pretrained:
        model_dict = model.state_dict()
...
def resnext101_32x8d(pretrained=False, progress=True, **kwargs):
    ...
    #                pretrained, progress, **kwargs)
    model = ResNet(Bottleneck, [3, 4, 23, 3], pretrained, progress, **kwargs)
    if pretrained:
        model_dict = model.state_dict()
