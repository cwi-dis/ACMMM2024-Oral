...
def knn_patch(pcd_name, patch_size = 2048):
    ...
    for i in  range(point_size):
        ...
        patch_list.append(np.concatenate((np.asarray(points)[idx[:], :], np.asarray(pcd.colors)[idx[:], :]), axis=1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', type=str, default = '/LSPCQA/lspcqa_patch_2048_color/') # path to the output patches
    parser.add_argument('--dataset', type=str, default = 'LSPCQA') # path to the output patches
    parser.add_argument('--modality', type=str, default = '3D_texture') # path to the output patches
